...
def load_ground_truth(dataset, ground_truth_path):
    ...
    relevant_cols_df = pd.read_csv(file_path, dtype={"case_id": str})[['case_id','event','follow_up_years','isup']]
...
def compute_global_cox_metrics(preds_df, gt_df):
    ...
    uni.fit(df[['isup', 'follow_up_years', 'event']],
            duration_col='follow_up_years',
            event_col='event')
    
    hr_u_c = uni.hazard_ratios_['isup']
    lo_u_c = uni.summary.loc['isup', 'exp(coef) lower 95%']
    hi_u_c = uni.summary.loc['isup', 'exp(coef) upper 95%']
    p_u_c  = uni.summary.loc['isup', 'p']
    
    # Multivariate Cox (prediction + ISUP)
    multi = CoxPHFitter()
    multi.fit(df[['prediction', 'isup', 'follow_up_years', 'event']],
              duration_col='follow_up_years',
              event_col='event')
    ...
    hr_m_c = multi.hazard_ratios_['isup']
    lo_m_c = multi.summary.loc['isup', 'exp(coef) lower 95%']
    hi_m_c = multi.summary.loc['isup', 'exp(coef) upper 95%']
    p_m_c = multi.summary.loc['isup', 'p']
    
    data_ai_capra_c = df[['prediction','isup', 'follow_up_years', 'event']]
    
    merged[['case_id','prediction','isup', 'follow_up_years', 'event']].copy().to_csv(
        "/data/temporary/leopard/source/evaluation/results/ensembled_predictions_all_datasets_isup_gt.csv")
    

    #c_index_ai_capra = concordance_index(df['follow_up_years'],
    #-multi.predict_partial_hazard(df[['prediction', 'isup', 
...
def format_global_results(vals):
    ...
    rows.append({
            ...
            'C-index AI Ensemble + ISUP': vals['c_index_ai_capra'],
            'C-index CI lower AI Ensemble + ISUP': vals['c_index_ai_capra_l'],
            'C-index CI upper AI Ensemble + ISUP': vals['c_index_ai_capra_u'],
...
def save_global_results(df, output_dir):
    ...
    # CSV
    df.reset_index().to_csv(os.path.join(output_dir, 'total_cox_metrics_isup_global.csv'), index=False)
    # LaTeX
    caption = f"Cox PH models on combined datasets (n={df['n'].max()} total cases)."
    with open(os.path.join(output_dir, 'total_cox_metrics_isup_global.tex'), 'w') as f:
...
def compute_ensemble_coxph(predictions, datasets, ground_truth_path):
    ...
    for ds_dict in datasets:
        ...
        capra_s  = zscore(sub['isup'].values)
...
def save_results(results, output_dir):
    os.makedirs(output_dir, exist_ok=True)
    results.to_csv(os.path.join(output_dir, 'all_ensemble_isup_c_index_results.csv'), index=False)
    with open(os.path.join(output_dir, 'all_ensemble_isup_c_index_results.tex'), 'w') as f:
