...
def compute_pr_data(pred_df, gt_df, horizon_years=5):
    """
    Returns PR curve data for AI / CAPRA-S / CAPRA-S + AI at a chosen horizon (years).
    """
...
def plot_precision_recall_subplots(pr_by_dataset, out_dir, horizon_years=2):
    ...
    fig.tight_layout()
    out_path = os.path.join(out_dir, f"precision_recall_all_datasets_{horizon_years}y.png")
...
def compute_and_save(preds, datasets, gt_dir, out_dir, cfg):
    horizon_years = 10  # <-- choose PR horizon here (e.g., 2, 3, 5, etc.)
    ...
    if ap_rows:
        ...
        out_csv = os.path.join(out_dir, f"average_precision_summary_{horizon_years}y.csv")
